...
class QuickSourceSpec(BaseModel):
    ...
    source_type: Optional[str] = Field(
        default=None,
        json_schema_extra={"enum": list(connector_registry().source_types())},
    )
...
def infer_source_type(location: str) -> SourceType:
    """Infer a connector using the shared platform connector registry."""
    return connector_registry().infer_source_type(location)


def canonical_location(location: str) -> str:
    """Normalize locations for stable Source identity without resolving I/O."""
    return connector_registry().canonical_location(location)
...
def build_source_config(
    ...
    try:
        return connector_registry().build_source_config(source_type, location, extra)
...
def _source_location(source: Source) -> Optional[str]:
    try:
        return connector_registry().source_location(source)
    except ValueError:
        return None
