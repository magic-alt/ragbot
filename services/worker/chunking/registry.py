...
def resolve_chunking_spec(
    ...
    *,
    ...
    Existing Sources remain `ragbot/fixed` unless they explicitly opt into a
    framework adapter. Nested `chunking.chunk_size/overlap` takes precedence
    over the legacy top-level source fields.
    """
    raw = dict(config or {})
    ...
    strategy = str(raw.get("strategy") or _DEFAULT_STRATEGY[provider]).strip().lower()
