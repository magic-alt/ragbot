...
-- Adds indexes for common query patterns and aligns schema with model fields.

-- Chunk checksum index for deduplication lookups
CREATE INDEX IF NOT EXISTS idx_chunks_checksum ON chunks(checksum) WHERE checksum IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_jobs_tenant_source ON ingestion_jobs(tenant_id, source_id);
